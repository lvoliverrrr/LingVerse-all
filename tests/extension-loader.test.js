...
test('extension loader exposes manifest version and cache-busts helper injection', () => {
    ...
    assert.equal(result.documentElement.dataset.lingverseAutoMapInjectedVersion, '2.71.0');
    assert.equal(result.documentElement.dataset.lingverseAutoMapExtensionVersion, '2.71.0');
    assert.equal(result.appended.length, 1);
    assert.equal(result.appended[0].src, 'chrome-extension://lingverse/lingverse-explore-helper.user.js?v=2.71.0');
});
...
test('extension loader skips duplicate injection for the same extension version', () => {
    const documentElement = {
        dataset: {
            ...
            lingverseAutoMapInjectedVersion: '2.71.0'
        }
    };
    const result = runLoader({ documentElement });

    assert.equal(result.appended.length, 0);
    assert.equal(documentElement.dataset.lingverseAutoMapInjectedVersion, '2.71.0');
});
